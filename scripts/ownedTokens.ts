...
async function main() {
    ...
    let numOwned = await nftContract.balanceOf(accounts[0].address);
    for (let i = 0; i < numOwned; i++) {
        console.log(`this account owns token: ${await nftContract.tokenOfOwnerByIndex(accounts[0].address, i)}`);
    }
}
